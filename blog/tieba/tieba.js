...
async function loadBoards() {
    try {
        ...
        if (response.ok) {
            ...
            if (data) {
                const boardIds = JSON.parse(data);
                // 将ID数组转换为对象数组
                boards = [
                    { id: 'moren', name: '默认帖子' }
                ];
                boardIds.forEach(id => {
                    if (id !== 'moren' && id !== 'hidden') {
                        boards.push({ id, name: id });
                    }
                });
                updateBoardSelect();
            }
...
function searchBoards() {
    ...
    const filteredBoards = boards.filter(board =>
        ...
        board.id.toLowerCase().includes(searchTerm)
    );
    ...
    if (filteredBoards.length === 0) {
        boardSelect.innerHTML = '<option value="">未找到相关帖子</option>';
    } else if (!filteredBoards.some(board => board.id === currentBoard)) {
        // 如果当前帖子不在搜索结果中，切换到第一个结果
        currentBoard = filteredBoards[0].id;
        loadMessages(currentBoard);
    }
}
...
async function updateHiddenBoard() {
    ...
    try {
        ...
        await saveToAPI('hidden', boardIds);
...
// 管理员登录函数（升级版）
async function adminLogin(passwordInputId = 'adminPassword') {
...
function setupEventListeners() {
    ...
    // 搜索功能
    searchBtn.addEventListener('click', searchBoards);
