...
## 功能概述
通过调用安全API (`https://api.130923.xyz/api/tianditu-key`) 动态获取天地图API密钥，避免密钥在前端代码中硬编码泄露。
...
```html
<script src="https://api.130923.xyz/api/tianditu-key"></script>
```
...
async function initTiandituMap() {
    ...
    if (typeof TIANDITU_KEY === 'undefined') {
        // 动态加载密钥脚本
        await loadScript('https://api.130923.xyz/api/tianditu-key');
    }
...
## 迁移状态
- [x] API接口已部署：`https://api.130923.xyz/api/tianditu-key`
