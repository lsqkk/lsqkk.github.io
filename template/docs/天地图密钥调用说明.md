...
## 功能概述
通过调用安全API (`https://api.130923.xyz/api/keys?names=tianditu`) 动态获取天地图API密钥，避免密钥在前端代码中硬编码泄露。
...
```html
<script src="https://api.130923.xyz/api/keys?names=tianditu"></script>
```
...
async function initTiandituMap() {
    ...
    if (typeof TIANDITU_KEY === 'undefined') {
        // 动态加载密钥脚本
        await loadScript('https://api.130923.xyz/api/keys?names=tianditu');
    }
...
4.  **错误处理**：如果地图无法加载，请检查浏览器控制台是否有CORS或404错误，并确保 `api.130923.xyz` 域名可访问。
5.  **密钥更新**：如需更换密钥，只需在Vercel环境变量中更新 `TIANDITU_KEY` 的值，无需修改前端代码。

## 迁移状态
- [x] API接口已部署：`https://api.130923.xyz/api/keys?names=tianditu`
