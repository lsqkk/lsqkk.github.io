## 在项目中引入 Firebase RTDB 配置

### 功能概述
通过调用部署在 `api.lsqkk.space` 的安全接口，动态获取 Firebase 实时数据库（RTDB）的完整配置，并完成初始化。所有敏感配置信息已托管于 Vercel 环境变量中。

### 前提条件
1.  已部署并正常运行 `https://api.lsqkk.space/api/firebase-config` 接口。（已完成，正常运行中）
2.  需要加载的 Firebase SDK（如 `firebase-app.js`, `firebase-database.js`）已正确引入。（需在新项目html中引入）
