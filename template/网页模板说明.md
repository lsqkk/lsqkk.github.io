...
1. 基础颜色和样式：元素(如方框)的默认UI设计为浅白色、高透明、圆角液态玻璃，saturate值在120%左右。
2. 时刻保持简洁和大气：严格禁止使用任何花里胡哨的任何渐变、边缘线条等虽属于现代UI但烂大街的庸俗页面设计，应当力求简洁、大气、用更少的样式表达更好的视觉效果。
3. 交互式：允许使用极少量的简洁悬浮交互（如轻微亮度变化、轻微上升、光影追随、Y轴翻转）等，但不宜太多，需根据提示词具体对页面美观或简洁的需求进行调整。
4. 响应式：在你认为需要的时候，进行适当响应式设计，以优化移动端适配。
5. 背景：默认背景图片（提示词中为博客背景）使用图片地址为：/assets/img/bg.png，当用户明确说明（使用博客背景/使用默认背景）或未指定任何背景时使用此地址。
