...
### 功能概述
通过调用安全API (`https://api.lsqkk.space/api/admin-auth`) 验证管理员身份。密码哈希校验逻辑完全在服务端进行，前端仅提交哈希值。

### 前提条件
1.  已部署并正常运行 `https://api.lsqkk.space/api/admin-auth` 接口。
2.  正确的管理员密码哈希已存入Vercel环境变量 `ADMIN_PASSWORD_HASH`。
3.  该API已配置为允许你的前端域名（`localhost`、`lsqkk.github.io`）进行跨域调用。

### 引入步骤与样例代码

#### 1. 前端登录函数模板
以下是一个包含基础错误处理和UI状态管理的异步登录函数模板，需要根据页面元素ID进行调整。
```javascript
/**
 * 管理员登录函数（安全API版本）
...
*   **API 安全**：该接口已配置来源检查，仅允许你指定的域名调用，无需在前端担心密钥泄露。
