...
// SVG icon helper — nav 专用图标内联嵌入，无需额外加载 fa-icons.js
function svgIcon(name) {
  var ICONS = {
    ...
    'id-badge': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" width="1em" height="1em" fill="currentColor" style="vertical-align:middle;display:inline-block"><path d="M320 0H64C28.65 0 0 28.65 0 64v384c0 35.35 28.65 64 64 64h256c35.35 0 64-28.65 64-64V64C384 28.65 355.3 0 320 0zM336 448c0 8.836-7.164 16-16 16H64c-8.836 0-16-7.164-16-16V64c0-8.838 7.164-16 16-16h64V64c0 17.67 14.33 32 32 32h64c17.67 0 32-14.33 32-32V48h64c8.836 0 16 7.162 16 16V448zM192 288c35.35 0 64-28.65 64-64s-28.65-64-64-64C156.7 160 128 188.7 128 224S156.7 288 192 288zM224 320H160c-44.18 0-80 35.82-80 80C80 408.8 87.16 416 96 416h192c8.836 0 16-7.164 16-16C304 355.8 268.2 320 224 320z"/></svg>',
...
// bodyClass 可选，用于指定不同菜单类型的 body 可见类名（默认 'nav-hover-body'）
function _positionFixedDropdown(triggerEl, menuEl, bodyClass) {
    if (!triggerEl || !menuEl) return;
    bodyClass = bodyClass || 'nav-hover-body';

    // 取消任何尚未完成的淡出清理
    if (menuEl._resetTimer) {
        clearTimeout(menuEl._resetTimer);
        menuEl._resetTimer = null;
    }
    ...
    menuEl.classList.add(bodyClass);
    ...
    // 触发重排后清除内联透明度/变形 → CSS 类值接管 → transition 执行淡入
...
function _resetFixedDropdown(menuEl, bodyClass) {
    if (!menuEl) return;
    bodyClass = bodyClass || 'nav-hover-body';
    ...
    // 动画完成后清理并恢复
    menuEl._resetTimer = setTimeout(() => {
        menuEl.classList.remove(bodyClass);
...
function initializeLanguagePickers() {
    ...
    const closeAll = () => {
        pickers.forEach(picker => {
            ...
            // 恢复语言菜单定位（可能在 body 上，用存储的引用查找）
            const menu = picker.querySelector('.language-menu') || picker._bodyMenu;
            if (menu instanceof HTMLElement) {
                _resetFixedDropdown(menu, 'language-menu-body');
            }
            picker._bodyMenu = null;
        });
    };
            ...
            trigger.addEventListener('click', (event) => {
                ...
                if (!isOpen) {
                    ...
                    if (menu instanceof HTMLElement && picker.closest('.header-nav-container')) {
                        picker._bodyMenu = menu;
                        _positionFixedDropdown(trigger, menu, 'language-menu-body');
                    }
                }
            });
        }
