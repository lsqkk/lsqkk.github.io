...
const utils = {
    ...
    exportToJson: (data, filename) => {
        ...
        a.href = url;
        a.download = filename || '/assets/pages/tool/random-wheel/wheel-data.json';
...
const dataManager = {
    ...
    loadDefaultWheels: async function () {
        try {
            // 尝试从JSON文件加载
            const response = await fetch('/assets/pages/tool/random-wheel/data/default-wheels.json');
            if (response.ok) {
...
const uiManager = {
    ...
    importWheelsJson: function () {
        ...
        input.type = 'file';
        input.accept = '/assets/pages/tool/random-wheel/.json';
