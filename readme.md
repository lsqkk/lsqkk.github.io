...
- **自定义网站管理工具**：开发了 **Quark 命令行工具**，通过 Python 包形式封装博客管理操作，实现了 `quark build`（站点构建）、`quark map`（站点地图）、`quark ppush`（一键构建+地图+推送）等命令，提升本地管理效率。
...
- [文章](https://lsqkk.github.io/posts)：日常分享、架构设计、开发心得、技术选型思考。文章源文件使用 Markdown，构建时由 Astro 模板生成静态页面并同步发布到根目录。评论系统使用 [giscus](https://giscus.app/zh-CN) 实现。
...
3. **本地构建站点**：运行 `quark build`。该命令会执行 `posts.json` 生成、Astro 构建、并将构建结果发布到仓库根目录（适配 GitHub Pages 根目录部署）。

4. **推送文章**：运行 `quark ppush` 以推送文章。这会自动执行站点构建（`quark build`）、更新站点地图（`quark map`），并创建一个名为“更新 - 更新了文章”的提交记录。
