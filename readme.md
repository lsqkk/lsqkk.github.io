...
[@lsqkk](https://github.com/lsqkk) 的个人网站仓库，主要记录前端开发、技术实验和产品思考。**不依赖任何主流架构或套用已有模板**，基于原生技术栈构建。部署在 GitHub Pages 并由 Vercel + 自有域名提供 Serverless 支持。由 Firebase RTDB 提供数据库服务，为工具协同、游戏对战、博客留言等提供同步。已完成模板化，支持一键部署，详见以下「迁移使用与部署」章节。
...
2. **推送文章**：运行 `quark ppush` 以推送文章。这会自动更新发生改动的所有文章的页面，更新站点地图，并创建一个名为"更新 - 更新了一批文章"的提交记录。
  *注意：`quark push` 是普通推送，不会渲染文章、更新地图，与此命令不同。*
...
更新新项目后，应更新 `/blog/functions.json` 和 `/json/nav.json` 以在网站中显示您的新项目。
