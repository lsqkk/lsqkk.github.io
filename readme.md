...
- [文章](https://lsqkk.github.io/posts)：日常分享、架构设计、开发心得、技术选型思考。使用可维护的文章 `html` 模板和 `py` 脚本自动生成/更新文章页面，实现类似网页生成器的效果。评论系统使用 [giscus](https://giscus.app/zh-CN) 实现。
...
基于你的需求，我为你撰写了一段关于“使用/部署”的文字，可以放在 **“主要技术特点”** 部分之后，或者单独作为一个小节插入在“Quark Blog 功能板块”之前，这样逻辑上更连贯：

## 迁移使用与部署

本项目为**个人从零开发的纯原生技术栈项目**，初衷是探索静态站点下的动态能力边界，因此**并未采用通用模板或框架**。代码结构、配置方式和构建流程均围绕个人需求定制，虽已尽力提高模块化与可维护性，但若你想 Fork 并部署自己的版本，可能会遇到以下情况：

- **配置较为分散**：部分功能（如 Firebase、Vercel 函数、Agora 等）依赖环境变量或私有配置，需手动替换为你的服务信息。
- **项目结构个性化**：文章生成、工具集、实验功能等目录结构可能不完全符合通用模板习惯。
- **构建流程非标准化**：部分页面依赖自定义 Python 脚本或本地预处理，需确保运行环境支持。
...
```
/api            - 个人私有API，可直接删除
/tool           - 除 index 文件可删除其他子目录
/games          - 除 index 文件可删除其他子目录
/blog           - 除 index 文件可删除其他子目录
/a              - 夸克博客前端实验室，可直接删除
其他 Quark Blog 自有功能等
…
```
