...
`quark qqexport` 从 QQ 空间获取自己的说说动态，合并更新到 `assets/data/dt.json`。
...
- **自动确定日期范围**：读取 `assets/data/dt.json` 中最新一条说说的日期，从次日开始导出直到当天，无需手动配置。
- **输出写入**：新内容追加到 `assets/data/dt.json` 数组头部（最新在前）。
