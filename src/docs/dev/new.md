...
- [x] 工具页新增「找不同辅助」
完成时间：2026-08-16
完成说明：新增 `/tool/spotdiff` 工具，上传找不同游戏截屏后，用 5 条参考线（左右模式：A左缘 / B左缘·分界 / 右缘 + 顶部 / 底部两条正交线；上下模式反之）精确框出 A/B 两图，按原始分辨率逐像素做差（max(|ΔR|,|ΔG|,|ΔB|)，不做降采样），自适应对比度输出清晰的黑白差异图（黑=相同、白/亮=差异，无需手动调阈值）。仅保留「差异图 / 原图A」切换与下载全分辨率 PNG；参考线位置持久化到 localStorage（`spotdiff-settings-v2`），并已在 `assets/pages/tool/tool.json` 的「图片」分类登记。
