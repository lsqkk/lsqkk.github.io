...
- [x] 工具页新增「找不同辅助」
完成时间：2026-08-16
完成说明：新增 `/tool/spotdiff` 工具，上传找不同游戏截屏后，通过拖拽主/辅参考线与分隔线（支持左右并列、上下堆叠两种模式）分割 A/B 两图，逐像素作差并高亮差异。提供热力图、差值灰度、高亮标记、区域框选（连通域聚类+编号计数）四种结果视图，附带自动对齐、降噪、阈值调节、缩放平移、下载 PNG、粘贴图片等功能；参考线位置与设置持久化到 localStorage（`spotdiff-settings-v1`），并已在 `assets/pages/tool/tool.json` 的「图片」分类登记。
