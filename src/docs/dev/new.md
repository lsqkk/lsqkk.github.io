...
- [x] 工具页新增「找不同辅助」
完成时间：2026-08-16
完成说明：新增 `/tool/spotdiff` 工具，上传找不同游戏截屏后，用 5 条参考线（左右模式：A左缘 / 分界(红线，=B左缘=A右缘) / B右缘 + 顶部 / 底部两条正交线；上下模式反之）精确框出 A/B 两图，按原始分辨率逐像素做差（max(|ΔR|,|ΔG|,|ΔB|)，不做降采样），自适应对比度输出清晰的黑白差异图（黑=相同、白/亮=差异，无需手动调阈值）。线可任意拖动、自动按位置排序。差异图下方提供「尝试自动对齐」（分界线附近 ±10px 内逐像素暴力比较，取界面"差异像素"数最小的位置，1/2 尺度采样加速、不做粗精修与降噪，"对齐中…"按钮反馈与分界线移动量提示）与「微调分界线」（0.2/0.5/1/2/5/10px 步长按钮或 Ctrl+方向键，分割位置保留浮点 + 平滑采样，支持亚像素对齐）用于手动/自动对齐。视图为「差异图 / 原图+标红」：原图标红仅标记差异亮度 ≥60% 的强差异像素，自动滤掉微弱 JPEG 噪点；支持下载全分辨率 PNG。参考线与步长持久化到 localStorage（`spotdiff-settings-v2`），并已在 `assets/pages/tool/tool.json` 的「图片」分类登记。
