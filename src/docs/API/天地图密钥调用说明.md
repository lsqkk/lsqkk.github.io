...
## 功能概述
通过调用安全 API（`__API_BASE__/api/keys?names=tianditu`）动态获取天地图 API 密钥，避免密钥在前端代码中硬编码泄露。
...
```html
<script src="__API_BASE__/api/keys?names=tianditu"></script>
```
...
async function initTiandituMap() {
    ...
    if (typeof TIANDITU_KEY === 'undefined') {
        // 动态加载密钥脚本
        await loadScript('__API_BASE__/api/keys?names=tianditu');
    }
...
3.  **缓存机制**：API响应设置了1小时缓存，密钥更新后前端可能不会立即生效。如需强制更新，可在API链接后添加版本号：`/api/keys?names=tianditu&v=2`
4.  **错误处理**：如果地图无法加载，请检查浏览器控制台是否有 CORS 或 404 错误，并确保你的 API 域名可访问。
5.  **密钥更新**：如需更换密钥，只需在Vercel环境变量中更新 `TIANDITU_KEY` 的值，无需修改前端代码。

> ⚠ **白名单提示**：域名变更需同步更新 `api/keys.js` 的允许列表，否则密钥不会被注入。

## 迁移状态
- [x] API 接口已部署：`__API_BASE__/api/keys?names=tianditu`
