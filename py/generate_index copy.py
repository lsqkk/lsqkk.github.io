...
def assemble_main_content(config, template_dir):
    ...
    for key, filename in main_templates:
        if config["main_content"].get(key, True):
...
def assemble_sidebar_content(config, template_dir):
    ...
    # 收集所有需要显示的内容
    displayed_contents = []
    for key, filename in sidebar_templates:
        if config["sidebar"].get(key, True):
            template_path = os.path.join(template_dir, filename)
            if os.path.exists(template_path):
...
def assemble_footer_content(config, template_dir):
    ...
    for key, filename in footer_templates:
        if config["footer"].get(key, True):
            template_path = os.path.join(template_dir, filename)
            if os.path.exists(template_path):
                footer_content_parts.append(load_template(template_path))
            else:
                print(f"警告: 模板文件 {template_path} 不存在")
