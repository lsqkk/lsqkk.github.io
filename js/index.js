...
// 手动添加之前的统计数据
function addPreviousStatistics() {
    // 之前的统计数据：845人访问过5609次
    const previousUV = 845;
    const previousPV = 5609;
    ...
    const checkBusuanzi = setInterval(() => {
        ...
        if (uvElement && pvElement && uvElement.innerText && pvElement.innerText) {
            // 获取当前不蒜子统计的值
            const currentUV = parseInt(uvElement.innerText) || 0;
            const currentPV = parseInt(pvElement.innerText) || 0;

            // 加上之前的统计数据
            const totalUV = currentUV + previousUV;
            const totalPV = currentPV + previousPV;

            // 更新显示
            uvElement.innerText = totalUV.toLocaleString();
            pvElement.innerText = totalPV.toLocaleString();

            // 更新存储函数，让它存储总值
            updateStorageFunction(totalUV, totalPV);
...
// 更新存储函数，让它存储包含历史数据的总值
function updateStorageFunction(baseUV, basePV) {
    // 重写storeStatistics函数
    window.storeStatistics = function () {
        const uvElement = document.getElementById('busuanzi_value_site_uv');
        const pvElement = document.getElementById('busuanzi_value_site_pv');
        ...
        if (uvElement && pvElement) {
            ...
            localStorage.setItem(dateKey, JSON.stringify({
                uv: parseInt(uvElement.innerText.replace(/,/g, '')),
                pv: parseInt(pvElement.innerText.replace(/,/g, '')),
                timestamp: today.getTime()
            }));
        }
    };
...
document.addEventListener('DOMContentLoaded', function () {
    addPreviousStatistics();
});

// 同时监听不蒜子数据变化，确保后续更新也包含历史数据
new MutationObserver(() => {
    const uvElement = document.getElementById('busuanzi_value_site_uv');
    const pvElement = document.getElementById('busuanzi_value_site_pv');

    if (uvElement && pvElement && uvElement.innerText && pvElement.innerText) {
        const currentUV = parseInt(uvElement.innerText.replace(/,/g, '')) || 0;
        const currentPV = parseInt(pvElement.innerText.replace(/,/g, '')) || 0;

        // 如果数值小于历史数据，说明不蒜子重置了，需要重新加上
        if (currentUV < 845 || currentPV < 5609) {
            addPreviousStatistics();
        }
    }
}).observe(document.getElementById('busuanzi_value_site_pv'), {
